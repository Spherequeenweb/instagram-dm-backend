...
# CORS - frontend se connect hone deta hai
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def verify_instagram(creds: InstagramCreds):
    ...
    try:
        ...
        }).eq("id", sb.table("settings").select("id").limit(1).execute().data[0]["id"]).execute()
...
def run_campaign(req: RunCampaignRequest):
    ...
    try:
        ...
        if remaining <= 0:
            return {"success": False, "message": "Aaj ka daily limit complete ho gaya! Kal try karo."}
        ...
        for p in prospects:
            try:
                ...
                failed += 1
                errors.append(f"{p.get('name', 'Unknown')}: {str(e)}")
                sb.table("dm_logs").insert({
                    "prospect_id": p["id"],
                    "message_type": "initial",
...
def run_followups():
    ...
    try:
        ...
        # Jo prospects follow-up ke liye due hain
...
5. Popup aaye toh phir **"Commit changes"** dabao

---

#### File 2: `requirements.txt`

1. **"Add file"** → **"Create new file"** dabao
...
#### File 3: `Procfile` (capital P, koi extension nahi!)

1. **"Add file"** → **"Create new file"** dabao
2. File name: `Procfile` (exactly aise hi, capital P)
3. Yeh paste karo:
```
